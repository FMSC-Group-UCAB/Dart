...
class RequestAppointmentUseCase extends Observable {
  ...
  requestAppointment(Patient patient, Doctor doctor, AppointmentDate date,
    ...
    if (patient.holdType == HoldType.BADUSE ||
        doctor.holdType == HoldType.BADUSE) {
      throw Exception(
          'El sistema está bloqueado para el paciente o para el doctor por el mal uso del mismo.');
    }

    //Si el paciente tiene un hold por no tener suscripción activa.
    if (patient.holdType == HoldType.EXPIREDSUBSCRIPTION) {
      throw Exception(
          'No se puede solicitar una cita sin pagar la suscripción.');
    }

    //Si el paciente no tiene un hold, se crea la cita.
    this.appointment = Appointment.create(AppointmentId.create(1), patient,
        doctor, StatusType.PENDING, date, appointmentType, specialtyType);

    // Aqui es donde se almacena la informacion de la cita en el UserRepositoy
    print(
        'El paciente ${patient.firstName.value} solicitó exitosamente una cita de ${specialtyType.toString()} con el doctor ${doctor.firstName.value} para el día ${date.value}');
